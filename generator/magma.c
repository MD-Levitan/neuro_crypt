...
inline static uint8_t f4(uint8_t x)
{
	return k1[x & 15];
}
...
void magma_neuro_g5_primitive(struct crypto_magma_ctx *magma_ctx, const uint32_t n1, const uint32_t n2, uint32_t *y)
{
	magma_subkeys *subkeys = keys_magma_ctx(magma_ctx);
	uint32_t buf = 0;
