...
def init_one_layer_network(input_data: np.array, output_data: np.array, n_input: int, n_classes: int,
    ...
    with tf.compat.v1.Session() as sess:
        sess.run(init)
        train_dataset = np.array(input_data[:18 * len(input_data) // 20])
        train_values = np.array(output_data[:18 * len(input_data) // 20])

        test_dataset = np.array(input_data[-len(input_data) // 20:])
        test_values = np.array(output_data[-len(input_data) // 20:])
        ...
        if verbose:
            ...
            print("work time: {}s".format(timer))
            print("End Training with epoch {}\n\n\n".format(training_epochs))

        sess.close()

    return acc, los, acc1, los1, timer, predict_values, test_values
...
def init_multilayer_network(input_data: np.array, output_data: np.array, n_input: int, n_hidden: list,
    ...
    return acc, los, acc1, los1, timer, \
